...
<FirebaseApp {firebase}>
	<MaterialApp {theme}>
		<!--change theme button-->
		<Button on:click="{toggleTheme}">{theme == 'light' ? 'Dark mode': 'Light mode'} </Button>
    ...
    <User let:user let:auth>
      ...
      <Doc path={`userCourses/${user.uid}`} let:data={userCourse} let:ref={userCourseRef} log>

        <h2>{userCourse.title}</h2>

        <p>
          Document
          created at <em>{new Date(userCourse.createdAt).toLocaleString()}</em>
        </p>
		...
		<Collection path={db.collection("courses")} let:data={courses}>
				<CoursesAccordians {courses}></CoursesAccordians>
		</Collection>
		<ListCourses/>
	</MaterialApp>
</FirebaseApp>